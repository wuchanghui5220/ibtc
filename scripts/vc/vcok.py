    ...
    def get_detailed_path(self, is_top_routing: bool) -> str:
        ...
        if self.path_type == 'same_cabinet':
            ...
            route = [
                ...
                f"在纵向线槽向{'北' if target_y < source_y else '南'}走线 {dy:.2f}米至第{target_y}排",  # 修正：Y坐标大的在南边
...
def calculate_standard_cable_length(length: float) -> int:
    """
    根据规律计算标准线缆长度
    规律分析：
    - 区间边界：4, 8, 13, 18, 23, 28, 33, 38, 43, 48, 53, 58, 63, 68, 73, 78, ...
    - 标准长度：5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80, ...
...
class DataCenter:
    ...
    def find_all_conduit_paths(self, source_coords: Tuple[int, int, int], 
                              target_coords: Tuple[int, int, int]) -> List[Tuple[int, float]]:
        """找到所有可能的纵向线槽路径并返回路径长度列表"""
        if not self.vertical_conduits:
            return []
        
        source_x, source_y, source_u = source_coords
        target_x, target_y, target_u = target_coords
        ...
        # 计算垂直距离
        if self.config['is_top_routing']:
            v1 = ((self.config['total_u_height'] - source_u) * 
                 self.config['u_height'] + fixed_height)
            v2 = ((self.config['total_u_height'] - target_u) * 
                 self.config['u_height'] + fixed_height)
        else:
            v1 = source_u * self.config['u_height'] + fixed_height
            v2 = target_u * self.config['u_height'] + fixed_height
        
        paths = []
        for conduit_x in self.vertical_conduits:
            # 计算通过此纵向线槽的路径长度
            dx1 = abs(conduit_x - source_x) * self.floor_size  # 源设备到纵向线槽
            dy = abs(target_y - source_y) * self.floor_size    # 纵向线槽上的距离
            dx2 = abs(target_x - conduit_x) * self.floor_size  # 纵向线槽到目标设备
            
            total_length = dx1 + dy + dx2 + v1 + v2
            paths.append((conduit_x, total_length))
    ...
    def calculate_cable_length(self, source_device: str, target_device: str, 
        ...
        if target_datacenter is None:
            ...
            # 3. 不同排的设备 - 必须使用纵向线槽
            # 3. 不同排的设备 - 必须使用纵向线槽
            all_conduit_paths = self.find_all_conduit_paths(source_coords, target_coords)
            
            # 如果没有线槽，抛出错误（不同排必须有纵向线槽）
            if not all_conduit_paths:
                raise ValueError(f"不同排设备之间必须使用纵向线槽，但未找到可用的纵向线槽。源设备：{source_device}({source['x']},{source['y']})，目标设备：{target_device}({target['x']},{target['y']})")
            
            # 4. 使用最短的线槽路径
            best_conduit_x, best_total_length = all_conduit_paths[0]  # 已经按长度排序
            
            # 重新计算最短路径的详细信息
            dx1 = abs(best_conduit_x - source['x']) * self.floor_size
            dy = abs(target['y'] - source['y']) * self.floor_size
            dx2 = abs(target['x'] - best_conduit_x) * self.floor_size
            ...
            path_info = CablePathInfo(
                ...
                source_coords=source_coords,
                target_coords=target_coords,
                conduit_coords=(best_conduit_x, source['y']),  # 保存完整的线槽坐标
                ...
                cabinet_to_conduit_bottom=self.config['cabinet_to_conduit_bottom'],
                fixed_height_adjustment=self.config['fixed_height_adjustment'],
                source_conduit_x=best_conduit_x
            )
            return path_info
        ...
        # 跨机房布线 - 改进逻辑，考虑所有出口入口组合
            ...
            for source_exit_id, target_exit_id in itertools.product(source_exits.keys(), target_exits.keys()):
                ...
                if total_length < min_total_length:
                    ...
                    # 创建最佳路径信息 - 关键修复点：确保total_length正确包含机房间距离
                    best_path = CablePathInfo(
                        total_length=total_length,  # 这里已经包含了机房间距离
                        ...
                        source_coords=source_coords,
                        target_coords=target_coords,
                        horizontal_distance=source_to_exit['horizontal_distance'] + target_from_exit['horizontal_distance'],
                        ...
                        room_distance=room_dist,  # 机房间距离单独存储，用于显示
                        exit_path=path_desc,
                        floor_size=self.floor_size,
                        cabinet_to_conduit_top=self.config['cabinet_to_conduit_top'],
                        cabinet_to_conduit_bottom=self.config['cabinet_to_conduit_bottom'],
        ...
        for sheet_name in sheet_names:
            ...
            for index, row in mapped_df.iterrows():
                ...
                try:
                    ...
                    # 计算总线缆长度（向上取整）- 修复点：确保总长度正确
                    total_length_rounded = math.ceil(path_info.total_length)
                    
                    # 计算标准线缆长度（使用特殊规律）
                    standard_length = calculate_standard_cable_length(path_info.total_length)
                    
                    # 验证跨机房布线的总长度计算
                    if path_info.path_type == 'cross_room':
                        # 验证总长度是否正确包含了机房间距离
                        expected_total = (path_info.vertical_distance1 + 
                                        path_info.vertical_distance2 + 
                                        path_info.horizontal_distance + 
                                        path_info.room_distance)
                        
                        # 如果计算不一致，使用预期的总长度
                        if abs(path_info.total_length - expected_total) > 0.01:
                            print(f"Warning: Total length mismatch for {source_device} -> {target_device}")
                            print(f"Calculated: {path_info.total_length:.2f}, Expected: {expected_total:.2f}")
                            path_info.total_length = expected_total
                            total_length_rounded = math.ceil(expected_total)
                            standard_length = calculate_standard_cable_length(expected_total)
                    ...
                    result_dict = {
                        ...
                        '总线缆长度(米)': total_length_rounded,  # 向上取整，已确保包含机房间距离
                        '标准线缆长度(米)': standard_length,  # 使用特殊规律计算
...
if __name__ == "__main__":
    input_file = "topology_128_servers-updated.xlsx"
    exits_config_file = "exits_config.xlsx"
    conduits_file = "conduits_config.xlsx"
    # 生成当前时间戳，格式为 YYYYMMDD_HHMMSS
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    # 在 output_file 文件名中插入时间戳
    output_file = f"cable_length_results_{timestamp}.xlsx"
