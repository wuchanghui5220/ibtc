...
def round_up_to_nearest_5(value):
    """将值向上取整为5的倍数"""
    return 5 * ((int(value) + 4) // 5)

# 在process_cable_calculations函数中，在创建result_dict时添加新列
        ...
        for sheet_name in sheet_names:
            ...
            for index, row in mapped_df.iterrows():
                ...
                try:
                    ...
                    # 计算原始线缆长度和向上取整为5的倍数的长度
                    total_length = round(path_info.total_length, 2)
                    rounded_length = round_up_to_nearest_5(total_length)
                    ...
                    result_dict = {
                        ...
                        '总线缆长度(米)': total_length,
                        '标准线缆长度(米)': rounded_length,  # 新增列：向上取整为5的倍数的线缆长度
