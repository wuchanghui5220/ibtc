...
im = OffsetImage(logo, zoom=0.15)
ab = AnnotationBbox(im, (0.9,0.9), xycoords=ax1.transAxes, boxcoords="offset points", frameon=False)
ax1.add_artist(ab)
...
if ports == NDR200:
    ...
    plt.text(tx, ty - 0.42, str(spine_to_leaf_cables) + "+" + str(leaf_to_server_cables) + "(Max:" + str(max_leaf_to_server_cables) + ")" + "=" + str(all_cables), fontsize=10, transform=ax2.transAxes)
ax2.text(tx, ty - 0.56, "Leaf to Spine each line: " + str(leaf2spine_each_line), fontsize=10, transform=ax2.transAxes)
